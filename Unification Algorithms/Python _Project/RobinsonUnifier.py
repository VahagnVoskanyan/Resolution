...
def robOccursCheck1(x, t, sigma):
    stack = [t]  # Initial stack containing the term t
    
    while stack:
        t = stack.pop()
        if isinstance(t, str) and t == x:  # Check if x occurs in t
            return False
        if isinstance(t, str) and t not in sigma:  # If variable y is bound in sigma
            continue
        if isinstance(t, tuple):  # If t is a function term, push its subterms
            for y in t[1:]:
                if isinstance(y, str) and y not in sigma:
                    stack.append(y)
    return True
...
def robOccursCheck(x, t, sigma):
    ...
    while stack:
        ...
        if isinstance(t, str) and t == x:  # Check if x occurs in t
            return False
        if isinstance(t, str) and t in sigma:  # If variable y is bound in sigma
    ...
    while stack:
        ...
        if s != t:
            if isinstance(s, str) and isinstance(t, str):
                ...
                return "failure"  # One is a function, the other is a variable
        else:
            continue
